export const BACKEND_WORKING = 'Backend funcionando 🚀'
export const SERVER_RUNNING = 'Servidor rodando'

export const DB_CONNECTED_SUCCESS = 'Banco de Dados conectado com sucesso.'

export const DEFAULT_MESSAGE_SUCCESS = 'Operação realizada com sucesso.'
export const LOGIN_SUCCESS = 'Login realizado com sucesso.'
export const RUNNING = 'Executando '

export const CONNECTING_MESSAGE = 'Conectando ao backend...'
export const LOADING_CONTENT = 'Carregando conteúdo...'
export const NO_CONTENT_AVAILABLE = 'Nenhum conteúdo disponível.'
export const LOAD_CONTENT = 'Conteúdo carregado:'
export const CONTACT_FORM_SUCCESS = 'Mensagem enviada com sucesso.'